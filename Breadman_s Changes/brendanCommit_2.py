...
import time
from _ast import If
from test.test_urlparse import RFC1808_BASE
...
class ComputerPlayer(Player):
    ...
    def mWC(self, highestCard):     # return first instance of minamal winnable card 
        leadingSuitHand = self.hand[self.suit2handDict.get(highestCard.getSuit())].getDeck()
        for card in leadingSuitHand:
            if(card.beat(highestCard,highestCard.getSuit())): return card
    ...
    def playCPU(self, leadingSuit, highestCard, curTurn, totTurn):
        ...
        if(len(leadingSuitHand) > 0):
            ...
            if(highestCard != "null" and trumpCard.beat(highestCard, leadingSuit) and rand.random()<self.aF):  # never trumps in beginning
                print("playing trump")
            ...
            elif(curTurn == totTurn):         # always wins in end (or add worth evaluation? if num of played hVal cards < totCards)
                print("playing mwc-end")
                self.choice = 3                                                 # forced play-win
                self.mWC(highestCard)
