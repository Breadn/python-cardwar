...
class Deck:
    class Card:
        ...
        def isValuable(self, bestAC, vT):
            # Check if highestCard's rank index is at least greater than 4 ranks below bestAC's rank index
    ...
    def playCPU(self, leadingSuit, highestCard, curTurn, totTurn):
        ...
        if(len(leadingSuitHand) > 0):
            ...
            # Overtake/Bait Play
                ...
                print("    DEBUG: value - %s > %s = %s" % (Deck.ranks.index(highestCard.getRank()),Deck.ranks.index(self.getBestAC(leadingSuit).getRank())-4,highestCard.isValuable(self.getBestAC(leadingSuit))))
