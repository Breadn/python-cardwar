...
class ComputerPlayer(Player):
    ...
    randCPUNameBank = ["Joe(CPU)", "Jin_Yang(CPU)", "NOTACPU(CPU)", "You(CPU)", "Big Chungus(CPU)", "Harambe(CPU)", "Chunky Boi(CPU)", "Hugh Mungus(CPU)", "CardSlayer(CPU)", "google_was_my_idea(CPU)","PizzaLeftovers(CPU)", "SunChips(CPU)", "JoeBiden(CPU)", "23904290348(CPU)", "awb9v8rycnw98(CPU)", "PancakesAreBetter(CPU)", "Derp(CPU)", "<InsertCreativeName>(CPU)", "Jimmy(CPU)"]
    ...
    def taunt(self):
        tauntList = ["Eat this -", "How'd you like them apples? -", "Get good -", "Good luck after that-", "Are you guys even trying? -", "I'd give up if I were you -", "Where'd the competition go? -"]
...
def main():
    while(True):
        ...
        deck = Deck()
        deck.fillClassic()
        for i in range(13): deck.shuffle()
